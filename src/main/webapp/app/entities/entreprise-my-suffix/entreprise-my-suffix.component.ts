...
@Component({
    selector: 'jhi-entreprise-my-suffix',
    templateUrl: './entreprise-my-suffix.component.html'
})
export class EntrepriseMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/entreprise-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/entreprise-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: EntrepriseMySuffix) {
        return item.id;
    }
